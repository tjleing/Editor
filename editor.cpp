...
void Editor::printBuff() {
  for(int i = wy; i<LINES-1+wy; ++i) {
    if(i >= (int)buff->lines.size()) {
      ...
      if(wx < (int)buff->lines[i].length()) {
        mvprintw(i-wy, 0, (buff->lines[i]).substr(wx, max(0,min((int)buff->lines[i].length(), COLS))).c_str());
      }
    }
    clrtoeol();
  }
...
void Editor::scrollToCorrect() {
  while(x < 0) {scrollLeft();x++;}
  while(x > COLS) {scrollRight();x--;}
  while(y < 0) {scrollUp();y++;}
  while(y > LINES - 1) {scrollDown();y--;}
}
...
void Editor::printStatusLine() {
  ...
  mvprintw(LINES-1, 0, tempstatus.c_str());
  clrtoeol();
  attron(A_BOLD);
